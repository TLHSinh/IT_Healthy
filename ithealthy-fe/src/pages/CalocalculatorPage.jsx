...
export default function CaloCalculatorPage() {
  // ===== AUTH =====
  const { user } = useContext(AuthContext);
  ...
  const pickMeal = (targetCalories) => {
    ...
    return { items, totalCalories: total };
  };
  ...
  const handleSaveHealth = async () => {
    ...
    // try {
    //   await axios.post("http://localhost:5000/api/HealthRecords", {
    //     customerId: user.customerId,
    //     gender,
    //     age,
    //     height,
  ...
  return (
    ...
    <div className="mb-5">
      ...
      <select
        className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-white focus:ring-2 focus:ring-blue-400 focus:outline-none transition"
        onChange={(e) => setGender(e.target.value)}
      >
        <option value="male">Nam</option>
        <option value="female">Nữ</option>
    ...
    {[
      ...
      <div className="mb-5" key={i}>
        ...
        <input
          ...
          className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-400 focus:outline-none transition"
          onChange={(e) => setter(+e.target.value)}
        />
      </div>
    ))}
              ...
              <>
                ...
                {user && (
                  <button
                    ...
                    disabled={!result}
                    className={`w-full py-3 rounded-xl mt-3 text-white ${
                      result
                        ? "bg-orange-500"
                ...
                {meals &&
                  Object.entries(meals).map(([key, meal]) => (
                    ...
                    >
                      <div className="flex justify-between mb-3">
                        <h3 className="font-bold capitalize">🍽️ {key}</h3>
                        <button
                          onClick={() => rerollMeal(key)}
                          className="w-9 h-9 bg-emerald-500 text-white rounded-full flex items-center justify-center"
                        >
