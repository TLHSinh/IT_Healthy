...
import { PlusCircle, Edit2, Trash2, RefreshCcw, Folders,Search } from "lucide-react";
...
const CategoryIngManagement = () => {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${BASE_URL}/category_ing`);
  ...
  const handleSave = async (form) => {
    try {
      if (selected) {
        await axios.put(
          `${BASE_URL}/category_ing/${selected.categoriesIngredientsId}`,
          form
        );
        toast.success("Cập nhật thành công!");
      } else {
        await axios.post(`${BASE_URL}/category_ing`, form);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/category_ing/${id}`);
    ...
    <div className="p-6">
      ...
      <div className="flex flex-wrap items-center justify-between mb-6 gap-3">
        ...
        <h2 className="flex items-center gap-3 text-3xl font-extrabold text-indigo-600">
          <Folders className="w-8 h-8 text-indigo-600" strokeWidth={2.5} />
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          >
            <PlusCircle className="w-5 h-5" /> Thêm danh mục
          </button>
        </div>
      </div>
          ...
          <tbody>
            {loading ? (
              ...
              currentPageData.map((cat, index) => (
                <tr
                  key={cat.categoriesIngredientsId}
                ...
                >
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex justify-center gap-2">
                      ...
                      <button
                        onClick={() =>
                          handleDelete(cat.categoriesIngredientsId)
                        }
...
export default CategoryIngManagement;
