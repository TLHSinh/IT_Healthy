...
const OrderModal = ({ order, customerAddress, onClose, printRef, onPrint }) => {
  ...
  // ⭐ ĐỊA CHỈ PICKUP = ĐỊA CHỈ CỬA HÀNG
  const pickupAddressParts = [
    order.streetAddress || order.storeStreetAddress,
    order.ward || order.storeWard,
    order.district || order.storeDistrict,
    order.city || order.storeCity,
  ].filter(Boolean);
                ...
                <div className="grid grid-cols-2 gap-6 p-6 text-gray-700 text-sm">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p>
                        {customerAddress?.receiverName || order.fullName || "—"}
                      </p>
                    </div>
                  </div>
                  ...
                  {/* ⭐ ĐỊA CHỈ CỬA HÀNG NHẬN, KHÔNG DÙNG customerAddress */}
                  <div className="col-span-2 flex items-start gap-2">
                    <MapPin className="mt-0.5 text-gray-500" size={18} />
                    <div>
                      <p className="font-semibold">Địa chỉ cửa hàng nhận</p>
...
const PickupOrdersPage = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter((o = {}) => {
      const hay = `${o.orderId ?? ""} ${o.fullName ?? ""} ${
        o.storeName ?? ""
      } ${
                ...
                >
                  ...
                  <div className="text-sm text-gray-700 mb-2">
                    <div className="flex items-center gap-2">
                      <User className="w-4 h-4" /> {o.fullName || "—"}
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                      <MapPin className="w-4 h-4" /> {o.storeName || "—"}
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                      <Calendar className="w-4 h-4" /> {safeDate(o.orderDate)}
                    </div>
                  </div>
